...
function useWater(deep: string, glow: string) {
  ...
  useFrame((_, dt) => {
    // Площадь аттракционов видна только в начале маршрута и при подъёме
    // над парком. На отрезках между ними камера стоит в террасе, зале и
    // шатре (z от -72 до -102) и смотрит ОТ площади, а колесо, карусель,
    // лодки и фонтан всё равно пересчитывались каждый кадр. Границы взяты
    // с запасом: 3.6 и 9.4 — заведомо внутри зданий.
    // Честно: выигрыш замером НЕ подтверждён — под удушением процессора
    // разница тонет в разбросе стенда. Правка оставлена потому, что
    // считать колесо, карусель, лодки и фонтан, когда их нет в кадре,
    // не нужно по определению; проверено, что в кадре они не замирают.
    if (fx.cam > 3.6 && fx.cam < 9.4) return
