...
export function CameraRig() {
  ...
  useFrame((_, delta) => {
    ...
    /**
     * Смягчённое оседание вместо чистого smoothstep.
     *
     * У smoothstep производная равна нулю на обоих концах: на КАЖДОЙ границе
     * кадра камера тормозила почти до остановки и разгонялась заново. Замер
     * внутри одного отрезка (скорость камеры на прокрученный пиксель,
     * нормированная на середину того же отрезка) давал у границы 0.02 на
     * отрезке 0, 0.11 на 8-м, 0.15 на 10-м — то есть камера шла в 7–50 раз
     * медленнее, чем в середине. При медленной прокрутке это читалось как
     * «оседание на кадре», ради чего и делалось, а при листании фликом — как
     * рывки и подвисания между секциями, при идеально ровных 16.7 мс на кадр.
     *
     * 0.3 оставляет заметное оседание, но минимальная скорость на границе
     * теперь 70% от равномерной, а не ноль. Путь между кадрами по-прежнему
     * прямой (lerpVectors), поэтому проверки проходимости из keyframes.ts
     * остаются в силе — камера не начинает срезать углы через стволы и стены.
     */
    const EASE = 0.3
    const raw = THREE.MathUtils.clamp(c - i, 0, 1)
    const t = raw + (smooth(raw) - raw) * EASE
    const a = KEYFRAMES[i]
