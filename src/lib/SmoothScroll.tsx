  ...
  useEffect(() => {
    ...
    if (reduced) {
      ...
      const lenis = new Lenis({
         ...
         *
         * ?nosync в адресе возвращает родную инерцию браузера. Это не отладка
         * ради отладки: замер плавности делался в headless на синтетических
         * касаниях, а на iOS родной скролл считает компоновщик, и syncTouch
         * (страница едет в основном потоке) может на живом телефоне
         * ощущаться ХУЖЕ. Сравнить два режима можно только на устройстве.
         */
        syncTouch: !new URLSearchParams(window.location.search).has('nosync'),
        autoRaf: false,
      })
      setLenis(lenis)
